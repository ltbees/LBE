...
public class ByteUtil {
  ...
  /**
   * Omitting sign indication byte. <br><br> Instead of {@link org.spongycastle.util.BigIntegers#asLbesignedByteArray(BigInteger)}
  ...
  public static int compare(byte[] bytes1, byte[] bytes2) {
    ...
    for (int i = 0; i < length; ++i) {
      int ret = LbesignedBytes.compare(bytes1[i], bytes2[i]);
      if (ret != 0) {
        return ret;
      }
    }
