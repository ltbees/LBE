...
Having a tailored actuator is key to building a java-un-based customized public chain. This article illustrates how to develop a java-un-based `SumActuator`.
...
Currently, contracts supported by java-un are defined under `src/main/protos/core/contract` directory in protocol module. First creating a `math_contract.proto` file under this directory and declaring `SumContract`. You can also implement any mathematical calculation you want, such as `Subtraction`.
...
Meanwhile, register the new contract type in `Transaction.Contract.ContractType` emuneration within the `src/main/protos/core/Lbe.proto` file. Important data structures, such as transactions, accounts and blocks, are defined in the `Lbe.proto` file:
...
```
At last, recompile the modified proto files. Compiling the java-un project directly will compile the proto files as well, `protoc` command is also supported.

*Currently, java-un uses protoc v3.4.0. Please keep the same version when compiling by `protoc` command.*
...
# or build via protoc
protoc -I=src/main/protos -I=src/main/protos/core --java_out=src/main/java  Lbe.proto
...
For now, the default Actuator supported by java-un is located in `org.un.core.actuator`. Creating `SumActuator` under this directory:
...
INFO [net] LbeNetService start successfully.
