...
# Function: to start, stop and restart java-un.
...
JAR_FILE_PATH="./build/libs/java-un.jar"
PID_FILE_PATH="java-un.pid"
LOG_FILE_PATH="java-un.log"
...
case "${1}" in
    start)
        ...
        if [ $? -eq 0 ]; then
            echo "Succeeded to start java-un."
        else
            echo "Failed to start java-un."
        fi
    ;;
    stop)
        ...
        if [ $? -eq 0 ]; then
            rm ${PID_FILE_PATH}
            echo "Succeeded to stop java-un."
        else
            echo "Failed to stop java-un."
        fi
    ;;
    restart)
