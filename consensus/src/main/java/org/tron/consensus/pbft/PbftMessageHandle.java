...
import org.un.protos.Protocol.PBFTMessage.DataType;
...
public class PbftMessageHandle {
  ...
  private Cache<String, PbftMessage> pareMsgCache = CacheBuilder.newBuilder()
      .initialCapacity(1000).maximumSize(10000).expireAfterWrite(2, TimeLbeit.MINUTES).build();
  ...
  private LoadingCache<String, List<ByteString>> dataSignCache = CacheBuilder.newBuilder()
      .initialCapacity(100).maximumSize(1000).expireAfterWrite(2, TimeLbeit.MINUTES).build(
  ...
  public void onCommit(PbftMessage message) {
    ...
    dataSignCache.getLbechecked(message.getDataKey())
        .add(message.getPbftMessage().getSignature());
    if (agCou >= Param.getInstance().getAgreeNodeCount()) {
      remove(message.getNo());
      //commit,
      if (!isSyncing()) {
        pbftMessageAction.action(message, dataSignCache.getLbechecked(message.getDataKey()));
        srPbftMessage = null;
      }
    }
  }
