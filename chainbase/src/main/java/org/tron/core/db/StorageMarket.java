...
public class StorageMarket {
  ...
  private long exchange_to_supply(boolean isLBE, long quant) {
    logger.info("isLBE: " + isLBE);
    long balance = isLBE ? dynamicPropertiesStore.getTotalStoragePool() :
...
//    }
  ...
  private long exchange_to_supply2(boolean isLBE, long quant) {
    logger.info("isLBE: " + isLBE);
    long balance = isLBE ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    logger.info("balance: " + balance);
...
//    }
  ...
  private long exchange_from_supply(boolean isLBE, long supplyQuant) {
    long balance = isLBE ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    supply -= supplyQuant;
  ...
  public long exchange(long from, boolean isLBE) {
    long relay = exchange_to_supply(isLBE, from);
    return exchange_from_supply(!isLBE, relay);
  }
