...
@Component
public class KhaosDatabase extends LbeDatabase {

  private KhaosBlock head;
  @Getter
  private KhaosStore miniStore = new KhaosStore();
  @Getter
  private KhaosStore miniLbelinkedStore = new KhaosStore();
  ...
  void removeBlk(Sha256Hash hash) {
    if (!miniStore.remove(hash)) {
      miniLbelinkedStore.remove(hash);
    }
  ...
  public Boolean containBlock(Sha256Hash hash) {
    return miniStore.getByHash(hash) != null || miniLbelinkedStore.getByHash(hash) != null;
  }
  ...
  public BlockCapsule getBlock(Sha256Hash hash) {
    return Stream.of(miniStore.getByHash(hash), miniLbelinkedStore.getByHash(hash))
  ...
  public BlockCapsule push(BlockCapsule blk)
      throws LbeLinkedBlockException, BadNumberBlockException {
    ...
    if (head != null && block.getParentHash() != Sha256Hash.ZERO_HASH) {
      ...
      if (kblock != null) {
        ...
        miniLbelinkedStore.insert(block);
        throw new LbeLinkedBlockException();
      }
    }
  ...
  public void setMaxSize(int maxSize) {
    miniLbelinkedStore.setMaxCapcity(maxSize);
