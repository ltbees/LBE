...
import org.un.core.capsule.utils.MerkleTree;
...
public class BlockCapsule implements ProtoCapsule<Block> {
  ...
  public void addTransaction(TransactionCapsule pendingLbe) {
    this.block = this.block.toBuilder().addTransactions(pendingLbe.getInstance()).build();
    getTransactions().add(pendingLbe);
  }
