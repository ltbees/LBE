...
import static org.un.core.vm.utils.MUtil.transfer;
import static org.un.core.vm.utils.MUtil.transferToken;
...
import org.un.common.runtime.InternalTransaction.ExecutorType;
...
import org.un.core.vm.LogInfoTriggerParser;
import org.un.core.vm.VM;
import org.un.core.vm.VMConstant;
import org.un.core.vm.VMUtils;
import org.un.core.vm.config.ConfigLoader;
import org.un.core.vm.config.VMConfig;
import org.un.core.vm.program.Program;
import org.un.core.vm.program.Program.JVMStackOverFlowException;
import org.un.core.vm.program.Program.OutOfTimeException;
import org.un.core.vm.program.Program.TransferException;
import org.un.core.vm.program.ProgramPrecompile;
...
public class VMActuator implements Actuator2 {
  ...
  @Getter
  @Setter
  private InternalTransaction.LbeType trxType;
  ...
  public void validate(Object object) throws ContractValidateException {
    ...
    trx = context.getLbeCap().getInstance();
    ...
    switch (contractType.getNumber()) {
      case ContractType.TriggerSmartContract_VALUE:
        trxType = LbeType.LBE_CONTRACT_CALL_TYPE;
        call();
        break;
      case ContractType.CreateSmartContract_VALUE:
        trxType = LbeType.LBE_CONTRACT_CREATION_TYPE;
        create();
        break;
      default:
        throw new ContractValidateException("Lbeknown contract type");
    }
  }
  ...
  public void execute(Object object) throws ContractExeException {
    ...
    try {
      if (vm != null) {
        ...
        if (LbeType.LBE_CONTRACT_CREATION_TYPE == trxType && !result.isRevert()) {
      ...
      if (Objects.isNull(result.getException())) {
        ...
        result.setException(new RuntimeException("Lbeknown Throwable"));
      }
  ...
  private void create()
    ...
    if (percent < 0 || percent > VMConstant.ONE_HLBEDRED) {
    ...
    try {
      ...
      ProgramInvoke programInvoke = programInvokeFactory
          .createProgramInvoke(LbeType.LBE_CONTRACT_CREATION_TYPE, executorType, trx,
  ...
  private void call()
    ...
    if (isNotEmpty(code)) {
      ...
      long vmShouldEndInUs = vmStartInUs + thisTxCPULimitInUs;
      ProgramInvoke programInvoke = programInvokeFactory
          .createProgramInvoke(LbeType.LBE_CONTRACT_CALL_TYPE, executorType, trx,
              tokenValue, tokenId, blockCap.getInstance(), repository, vmStartInUs,
              vmShouldEndInUs, energyLimit);
      if (isConstantCall) {
  ...
  public long getAccountEnergyLimitWithFixRatio(AccountCapsule account, long feeLimit,
    ...
    long sunPerEnergy = VMConstant.SLBE_PER_ENERGY;
  ...
  public long getTotalEnergyLimitWithFixRatio(AccountCapsule creator, AccountCapsule caller,
    ...
    if (Arrays.equals(creator.getAddress().toByteArray(), caller.getAddress().toByteArray())) {
      ...
      // which will cost the feeLimit LBE when the creator's frozen energy is 0.
    ...
    if (consumeUserResourcePercent <= 0) {
      ...
      if (consumeUserResourcePercent < VMConstant.ONE_HLBEDRED) {
        // creatorEnergyLimit =
        ...
        creatorEnergyLimit = min(
            BigInteger.valueOf(callerEnergyLimit)
                .multiply(BigInteger.valueOf(VMConstant.ONE_HLBEDRED - consumeUserResourcePercent))
  ...
  private long getTotalEnergyLimitWithFloatRatio(AccountCapsule creator, AccountCapsule caller,
    ...
    if (creatorEnergyLimit * consumeUserResourcePercent
        > (VMConstant.ONE_HLBEDRED - consumeUserResourcePercent) * callerEnergyLimit) {
      return Math.floorDiv(callerEnergyLimit * VMConstant.ONE_HLBEDRED, consumeUserResourcePercent);
    } else {
      return Math.addExact(callerEnergyLimit, creatorEnergyLimit);
    }
  }
