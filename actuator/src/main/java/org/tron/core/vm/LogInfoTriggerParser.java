...
public class LogInfoTriggerParser {
  ...
  public List<ContractTrigger> parseLogInfos(List<LogInfo> logInfos, Repository deposit) {
    ...
    for (LogInfo logInfo : logInfos) {
      ...
      byte[] contractAddress = TransactionTrace.convertToLbeAddress(logInfo.getAddress());
      ...
      String creatorAddr = StringUtil.encode58Check(
          TransactionTrace.convertToLbeAddress(contract.getInstance().getOriginAddress().toByteArray()));
    ...
    for (LogInfo logInfo : logInfos) {
      ...
      event.setLbeiqueId(txId + "_" + index);
