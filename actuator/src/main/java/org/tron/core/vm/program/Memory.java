...
public class Memory implements ProgramListenerAware {
  ...
  public byte[] read(int address, int size) {
    ...
    int chunkIndex = address / CHLBEK_SIZE;
    int chunkOffset = address % CHLBEK_SIZE;
  ...
  public void extend(int address, int size) {
    ...
    if (toAllocate > 0) {
      addChunks((int) ceil((double) toAllocate / CHLBEK_SIZE));
    }
  ...
  public int internalSize() {
    return chunks.size() * CHLBEK_SIZE;
  }
  ...
  private void addChunks(int num) {
    for (int i = 0; i < num; ++i) {
      chunks.add(new byte[CHLBEK_SIZE]);
    }
  }
}
