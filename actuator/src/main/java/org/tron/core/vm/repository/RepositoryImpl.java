...
import org.un.core.db.BlockIndexStore;
import org.un.core.db.BlockStore;
import org.un.core.db.KhaosDatabase;
...
public class RepositoryImpl implements Repository {
  ...
  public void putStorageValue(byte[] address, DataWord key, DataWord value) {
    address = TransactionTrace.convertToLbeAddress(address);
  ...
  public Storage getStorage(byte[] address) {
    ...
    ContractCapsule contract = getContract(address);
    if (contract != null && !ByteUtil.isNullOrZeroArray(contract.getLbeHash())) {
      storage.generateAddrHash(contract.getLbeHash());
    }
    return storage;
  }
