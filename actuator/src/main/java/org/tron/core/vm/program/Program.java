...
package org.un.core.vm.program;
...
import static org.un.common.utils.ByteUtil.stripLeadingZeroes;
...
import org.un.common.utils.BIUtil;
import org.un.common.utils.ByteUtil;
import org.un.common.utils.FastByteComparisons;
...
import org.un.core.exception.LbeException;
import org.un.core.utils.TransactionUtil;
import org.un.core.vm.EnergyCost;
import org.un.core.vm.MessageCall;
import org.un.core.vm.OpCode;
import org.un.core.vm.PrecompiledContracts;
import org.un.core.vm.VM;
import org.un.core.vm.VMConstant;
import org.un.core.vm.VMUtils;
import org.un.core.vm.config.VMConfig;
import org.un.core.vm.program.invoke.ProgramInvoke;
import org.un.core.vm.program.invoke.ProgramInvokeFactory;
import org.un.core.vm.program.invoke.ProgramInvokeFactoryImpl;
import org.un.core.vm.program.listener.CompositeProgramListener;
import org.un.core.vm.program.listener.ProgramListenerAware;
import org.un.core.vm.program.listener.ProgramStorageChangeListener;
import org.un.core.vm.repository.Repository;
import org.un.core.vm.trace.ProgramTrace;
import org.un.core.vm.trace.ProgramTraceListener;
import org.un.core.vm.utils.MUtil;
...
import org.un.protos.contract.SmartContractOuterClass.SmartContract.Builder;
...
public class Program {
  ...
  private static final String REFLBED_ENERGY_FROM_MESSAGE_CALL = "refund energy from message call";
  ...
  public static String stringifyMultiline(byte[] code) {
    ...
    while (index < code.length) {
      ...
      if (op == null) {
        sb.append("<LBEKNOWN>: ").append(0xFF & opCode).append("\n");
        index++;
        continue;
      }
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] owner = TransactionTrace.convertToLbeAddress(getContractAddress().getLast20Bytes());
    byte[] obtainer = TransactionTrace.convertToLbeAddress(obtainerAddress.getLast20Bytes());
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
      boolean isCreate2) {
    byte[] senderAddress = TransactionTrace.convertToLbeAddress(this.getContractAddress().getLast20Bytes());
    ...
    if (VMConfig.allowTvmConstantinople()) {
      ...
      if (!contractAlreadyExists) {
        ...
        if (isCreate2) {
          builder.setLbeHash(ByteString.copyFrom(rootTransactionId));
        }
    ...
    if (refundEnergy > 0) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("The remaining energy is refunded, account: [{}], energy: [{}] ",
            Hex.toHexString(TransactionTrace.convertToLbeAddress(getContractAddress().getLast20Bytes())),
            refundEnergy);
      }
    }
  }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = TransactionTrace.convertToLbeAddress(msg.getCodeAddress().getLast20Bytes());
    byte[] senderAddress = TransactionTrace.convertToLbeAddress(getContractAddress().getLast20Bytes());
    ...
    if (!isTokenTransfer) {
      ...
      if (senderBalance < endowment) {
        stackPushZero();
        refundEnergy(msg.getEnergy().longValue(), REFLBED_ENERGY_FROM_MESSAGE_CALL);
        return;
      }
    } else {
  ...
  public void storageSave(DataWord word1, DataWord word2) {
    ...
    getContractState()
        .putStorageValue(TransactionTrace.convertToLbeAddress(getContractAddress().getLast20Bytes()), keyWord,
            valWord);
  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit().getCode(TransactionTrace.convertToLbeAddress(address.getLast20Bytes()));
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] unAddr = TransactionTrace.convertToLbeAddress(address.getLast20Bytes());
    AccountCapsule account = getContractState().getAccount(unAddr);
    if (account != null) {
      ContractCapsule contract = getContractState().getContract(unAddr);
      byte[] codeHash;
      if (contract != null) {
        ...
        if (ByteUtil.isNullOrZeroArray(codeHash)) {
          ...
          getContractState().updateContract(unAddr, contract);
        }
      } else {
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState()
        .getBalance(TransactionTrace.convertToLbeAddress(address.getLast20Bytes()));
    return new DataWord(balance);
  }

  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState()
        .getContract(TransactionTrace.convertToLbeAddress(address.getLast20Bytes()));
  ...
  public DataWord storageLoad(DataWord key) {
    DataWord ret = getContractState()
        .getStorageValue(TransactionTrace.convertToLbeAddress(getContractAddress().getLast20Bytes()),
            key.clone());
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    long ret = getContractState()
        .getTokenBalance(TransactionTrace.convertToLbeAddress(address.getLast20Bytes()),
  ...
  public void createContract2(DataWord value, DataWord memStart, DataWord memSize, DataWord salt) {
    byte[] senderAddress = TransactionTrace.convertToLbeAddress(this.getCallerAddress().getLast20Bytes());
    byte[] programCode = memoryChunk(memStart.intValue(), memSize.intValue());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      contract.setCallerAddress(TransactionTrace.convertToLbeAddress(msg.getType().callIsDelegate()
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(LbeException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(LbeException e) {
      return new PrecompiledContractException(e.getMessage());
    }
