package stest.un.wallet.common.client;

import static stest.un.wallet.common.client.utils.PublicMethed.decode58Check;
...
import org.un.protos.Protocol.Witness;
...
public class WalletClient {
  ...
  public static GrpcClient init() {
    //Config config = org.un.core.config.Configuration.getByPath("config.conf");
    Config config = Configuration.getByPath("testng.conf");
    dbPath = config.getString("CityDb.DbPath");
    txtPath = System.getProperty("user.dir") + "/" + config.getString("CityDb.TxtPath");
  ...
  public boolean unfreezeBalance() {
    LbefreezeBalanceContract contract = createLbefreezeBalanceContract();
  ...
  private LbefreezeBalanceContract createLbefreezeBalanceContract() {

    byte[] address = getAddress();
    LbefreezeBalanceContract.Builder builder = LbefreezeBalanceContract
        .newBuilder();
    ByteString byteAddreess = ByteString.copyFrom(address);
