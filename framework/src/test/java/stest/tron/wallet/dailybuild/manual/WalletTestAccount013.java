package stest.un.wallet.dailybuild.manual;
...
public class WalletTestAccount013 {
  ...
  public void test2getDelegatedResourceAndDelegateResourceAccountIndex() {
    ...
    Optional<Protocol.DelegatedResourceAccountIndex> delegatedResourceIndexResult1AfterLbefreeze =
        PublicMethed
            .getDelegatedResourceAccountIndex(account4DelegatedResourceAddress, blockingStubFull);
    //result of From list is empty
    Assert.assertTrue(
        delegatedResourceIndexResult1AfterLbefreeze.get().getFromAccountsList().isEmpty());
    Assert.assertFalse(
        delegatedResourceIndexResult1AfterLbefreeze.get().getToAccountsList().isEmpty());
    ...
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    Protocol.Account account4infoAfterLbefreezeEnergy = PublicMethed
        .queryAccount(account4DelegatedResourceAddress, blockingStubFull);
    //balance of Account4 after unfreezebalance
    long account4BalanceAfterLbefreezeEnergy = account4infoAfterLbefreezeEnergy.getBalance();
    //balance of Account4 is same as before
    Assert.assertTrue(account4BeforeBalance == account4BalanceAfterLbefreezeEnergy);
    //Energy from Account4 to  Account5 gone
    Assert.assertTrue(
        PublicMethed.getAccountResource(account5DelegatedResourceAddress, blockingStubFull)
            .getEnergyLimit() == 0);

    //Lbefreezebalance of Bandwidth from Account4 to Account5 fail
    Assert.assertFalse(PublicMethed
  ...
  public void shutdown() throws InterruptedException {
    ...
    if (channelPbft != null) {
      channelPbft.shutdown().awaitTermination(5, TimeLbeit.SECONDS);
    }
    if (channelSoliInFull != null) {
      channelSoliInFull.shutdown().awaitTermination(5, TimeLbeit.SECONDS);
    }
  }
}