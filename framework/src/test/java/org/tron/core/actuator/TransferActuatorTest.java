...
import static org.un.core.config.Parameter.ChainConstant.TRANSFER_FEE;
...
public class TransferActuatorTest {
  ...
  private static final String To_ACCOLBET_INVALID;
  ...
  public void rightTransfer() {
    ...
    actuator.setChainBaseManager(dbManager.getChainBaseManager()).setAny(getContract(AMOLBET));
    ...
    try {
      ...
      Assert.assertEquals(owner.getBalance(), OWNER_BALANCE - AMOLBET - TRANSFER_FEE);
      Assert.assertEquals(toAccount.getBalance(), TO_BALANCE + AMOLBET);
  ...
  public void iniviateLbe() {
    ...
    try {
      ...
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert.assertEquals("Cannot transfer LBE to yourself.", e.getMessage());
      AccountCapsule owner = dbManager.getAccountStore()
          .get(ByteArray.fromHexString(OWNER_ADDRESS));
      AccountCapsule toAccount = dbManager.getAccountStore()
          .get(ByteArray.fromHexString(TO_ADDRESS));
  ...
  public void noExitOwnerAccount() {
    TransferActuator actuator = new TransferActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
        .setAny(getContract(100L, OWNER_ACCOLBET_INVALID, TO_ADDRESS));
  ...
  public void noExitToAccount() {
    TransferActuator actuator = new TransferActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
        .setAny(getContract(1_000_000L, OWNER_ADDRESS, To_ACCOLBET_INVALID));

    TransactionResultCapsule ret = new TransactionResultCapsule();
    try {
      AccountCapsule noExitAccount = dbManager.getAccountStore()
          .get(ByteArray.fromHexString(To_ACCOLBET_INVALID));
      ...
      dbManager.getAccountStore().delete(ByteArray.fromHexString(To_ACCOLBET_INVALID));
    }
  }
  ...
  public void insufficientFee() {
    ...
    AccountCapsule toAccountCapsule =
        new AccountCapsule(
            ByteString.copyFromUtf8("toAccount"),
            ByteString.copyFrom(ByteArray.fromHexString(To_ACCOLBET_INVALID)),
            AccountType.Normal,
            100L);
    dbManager.getAccountStore().put(ownerCapsule.getAddress().toByteArray(), ownerCapsule);
    dbManager.getAccountStore().put(toAccountCapsule.getAddress().toByteArray(), toAccountCapsule);
    ...
    TransferActuator actuator = new TransferActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
        .setAny(getContract(AMOLBET, OWNER_NO_BALANCE, To_ACCOLBET_INVALID));
