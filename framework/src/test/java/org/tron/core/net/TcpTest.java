...
import static org.un.core.net.message.MessageTypes.P2P_DISCONNECT;
import static org.un.core.net.message.MessageTypes.P2P_HELLO;
import static org.un.protos.Protocol.ReasonCode.DUPLICATE_PEER;
import static org.un.protos.Protocol.ReasonCode.FORKED;
import static org.un.protos.Protocol.ReasonCode.INCOMPATIBLE_CHAIN;
import static org.un.protos.Protocol.ReasonCode.INCOMPATIBLE_VERSION;

import com.google.common.cache.CacheBuilder;
import io.netty.buffer.ByteBuf;
import io.netty.buffer.Lbepooled;
...
import org.un.common.overlay.message.P2pMessage;
import org.un.common.overlay.message.P2pMessageFactory;
...
public class TcpTest {
  ...
  public TcpTest(LbeApplicationContext context) {
    channelManager = context.getBean(ChannelManager.class);
    manager = context.getBean(Manager.class);
    chainBaseManager = context.getBean(ChainBaseManager.class);
    pool = context.getBean(SyncPool.class);
    unNetDelegate = context.getBean(LbeNetDelegate.class);
  }
  ...
  public void errorMsgTest() throws InterruptedException {
    ...
    channel.writeAndFlush(
        Lbepooled.wrappedBuffer(ArrayUtils.add("nihao".getBytes(), 0, (byte) 1)))
  ...
  private void validResultCloseConnect(Channel channel) throws InterruptedException {
    ...
    Collection<PeerConnection> peerConnections = ReflectUtils
        .invokeMethod(unNetDelegate, "getActivePeer");
    for (PeerConnection peer : peerConnections) {
      peer.close();
    }
    ReflectUtils.setFieldValue(channelManager, "recentlyDisconnected",
        CacheBuilder.newBuilder().maximumSize(1000)
            .expireAfterWrite(30, TimeLbeit.SECONDS).recordStats().build());
  }

  private void validResultLbeCloseConnect() throws InterruptedException {
    int n = 0;
