...
import org.un.common.runtime.TVMTestResult;
import org.un.common.runtime.TvmTestUtils;
...
public class TransferFailedEnergyTest extends VMTestBase {
  ...
  contract EnergyOfTransferFailedTest {
      ...
      // InsufficientBalance
      function testTransferLbeInsufficientBalance() payable public{
          msg.sender.transfer(10);
      }

      function testSendLbeInsufficientBalance() payable public{
          msg.sender.send(10);
      }
      ...
      function testCallLbeInsufficientBalance(address payable caller) public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }

      function testCreateLbeInsufficientBalance() payable public {
          (new Caller).value(10)();
      }

      // NonexistentTarget
      function testTransferLbeNonexistentTarget(address payable nonexistentTarget) payable public {
      ...
      function testCallLbeNonexistentTarget(address payable nonexistentTarget) payable public {
          require(address(this).balance >= 10);
      ...
      // target is self
      function testTransferLbeSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.transfer(10);
      }

      function testSendLbeSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.send(10);
      }
  ...
  contract EnergyOfTransferFailedTest {
      ...
      function testCallLbeInsufficientBalance(address caller) payable public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }
      ...
      // NonexistentTarget
      function testTransferLbeNonexistentTarget(address nonexistentTarget) payable public {
          require(address(this).balance >= 10);
          nonexistentTarget.transfer(10);
      }
      ...
      function testCallLbeNonexistentTarget(address nonexistentTarget) public {
          require(address(this).balance >= 10);
          nonexistentTarget.call.value(10)(abi.encodeWithSignature("test()"));
      }
  ...
  */
  ...
  TestCase[] testCasesAfterAllowTvmConstantinop = {
      new TestCase("testTransferLbeSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSendLbeSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSuicideNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testTransferLbeNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testCallLbeNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
  };
  TestCase[] testCasesBeforeAllowTvmConstantinop = {
      new TestCase("testTransferLbeSelf()", Collections.emptyList(),
          true, contractResult.LBEKNOWN),
      new TestCase("testSendLbeSelf()", Collections.emptyList(),
          true, contractResult.LBEKNOWN),
      new TestCase("testSuicideNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), true, contractResult.LBEKNOWN),
      new TestCase("testTransferLbeNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), true, contractResult.LBEKNOWN),
      new TestCase("testCallLbeNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), true, contractResult.LBEKNOWN),
  ...
  TestCase[] testCasesInsufficientBalance = {
      new TestCase("testTransferLbeInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
      new TestCase("testSendLbeInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.SUCCESS),
      new TestCase("testCreateLbeInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
      new TestCase("testCallLbeInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
