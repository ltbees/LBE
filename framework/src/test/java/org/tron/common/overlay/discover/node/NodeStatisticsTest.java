package org.un.common.overlay.discover.node;
...
import org.un.common.overlay.discover.node.statistics.MessageStatistics;
import org.un.common.overlay.discover.node.statistics.NodeStatistics;
import org.un.common.overlay.message.DisconnectMessage;
import org.un.common.overlay.message.PongMessage;
...
public class NodeStatisticsTest {
  ...
  public void testNode() throws NoSuchFieldException, IllegalAccessException {
    ...
    Assert.assertEquals(Protocol.ReasonCode.LBEKNOWN, reasonCode);
  ...
  public void testMessage() {
    ...
    Assert.assertEquals(1, statistics.unInSyncBlockChain.getTotalCount());
    ...
    Assert.assertEquals(1, statistics.unOutBlockChainInventory.getTotalCount());

    InventoryMessage invMsgLbe =
        new InventoryMessage(new ArrayList<>(), Protocol.Inventory.InventoryType.LBE);
    String inventoryMessageStr = invMsgLbe.toString();
    Assert.assertNotNull(inventoryMessageStr);
    statistics.addTcpInMessage(invMsgLbe);
    statistics.addTcpOutMessage(invMsgLbe);
    ...
    Assert.assertEquals(1, statistics.unInBlockInventory.getTotalCount());

    FetchInvDataMessage fetchInvDataLbe =
        new FetchInvDataMessage(new ArrayList<>(), Protocol.Inventory.InventoryType.LBE);
    statistics.addTcpInMessage(fetchInvDataLbe);
    statistics.addTcpOutMessage(fetchInvDataLbe);
    ...
    Assert.assertEquals(1, statistics.unInLbeFetchInvData.getTotalCount());
    ...
    Assert.assertEquals(1, statistics.unInLbes.getTotalCount());
    ...
    long inBlockCount = statistics.unInBlock.getTotalCount();
