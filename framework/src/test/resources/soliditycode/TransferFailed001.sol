contract EnergyOfTransferFailedTest {
    ...
    // InsufficientBalance
    function testTransferLbeInsufficientBalance(uint256 i) payable public{
        msg.sender.transfer(i);
    }

    function testSendLbeInsufficientBalance(uint256 i) payable public{
        msg.sender.send(i);
    }
    ...
    function testCallLbeInsufficientBalance(uint256 i,address payable caller) public {
        caller.call.value(i)(abi.encodeWithSignature("test()"));
    }

    function testCreateLbeInsufficientBalance(uint256 i) payable public {
        (new Caller).value(i)();
    }

    // NonexistentTarget

    function testSendLbeNonexistentTarget(uint256 i,address payable nonexistentTarget) payable public {
        nonexistentTarget.send(i);
    }

    function testSendLbeRevert(uint256 i,address payable nonexistentTarget) payable public {
        nonexistentTarget.send(i);
        revert();
    }

    function testTransferLbeNonexistentTarget(uint256 i,address payable nonexistentTarget) payable public {
        nonexistentTarget.transfer(i);
    }

    function testTransferLberevert(uint256 i,address payable nonexistentTarget) payable public{
        nonexistentTarget.transfer(i);
        revert();
    }
    ...
    function testCallLbeNonexistentTarget(uint256 i,address payable nonexistentTarget) payable public {
    ...
    // target is self
    function testTransferLbeSelf(uint256 i) payable public{
        address payable self = address(uint160(address(this)));
        self.transfer(i);
    }

    function testSendLbeSelf(uint256 i) payable public{
        address payable self = address(uint160(address(this)));
        self.send(i);
    }
