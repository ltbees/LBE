...
contract FoMo3Dlong is F3Devents {
    ...
    returns (uint256, uint256, uint256)
    {
        ...
        if (now > round_[_rID].end && round_[_rID].ended == false && round_[_rID].plyr != 0)
        {
            ...
            (
            plyr_[_pID].win,
            (plyr_[_pID].gen).add(calcLbeMaskedEarnings(_pID, plyr_[_pID].lrnd)),
            plyr_[_pID].aff
            );
        }
    }
    ...
    returns (uint256, bytes32, uint256, uint256, uint256, uint256, uint256)
    {
        ...
        plyr_[_pID].win, //3
        (plyr_[_pID].gen).add(calcLbeMaskedEarnings(_pID, plyr_[_pID].lrnd)), //4
        plyr_[_pID].aff, //5
     ...
     */
    function calcLbeMaskedEarnings(uint256 _pID, uint256 _rIDlast)
    private
    view
    returns (uint256)
    {
    ...
    private
    {
        uint256 _earnings = calcLbeMaskedEarnings(_pID, _rIDlast);
        if (_earnings > 0)
        {
            // put in gen vault
