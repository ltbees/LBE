...
@Component
@Slf4j(topic = "API")
public class LbeFreezeAssetServlet extends RateLimiterServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      LbefreezeAssetContract.Builder build = LbefreezeAssetContract.newBuilder();
      JsonFormat.merge(contract, build, visible);
      Transaction tx = wallet
          .createTransactionCapsule(build.build(), ContractType.LbefreezeAssetContract)
          .getInstance();
