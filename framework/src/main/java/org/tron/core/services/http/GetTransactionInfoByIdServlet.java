...
public class GetTransactionInfoByIdServlet extends RateLimiterServlet {
  ...
  private static String convertLogAddressToLbeAddress(TransactionInfo transactionInfo,
      boolean visible) {
    if (visible) {
      ...
      for (Log log : transactionInfo.getLogList()) {
        ...
        if (oldAddress.length == 0 || oldAddress.length > 20) {
          ...
          logBuilder.setAddress(ByteString.copyFrom(TransactionTrace
              .convertToLbeAddress(newAddress)));
        }
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      if (reply != null) {
        response.getWriter().println(convertLogAddressToLbeAddress(reply, visible));
      } else {
