...
import static org.un.common.utils.Commons.getAssetIssueStoreFinal;
import static org.un.common.utils.Commons.getExchangeStoreFinal;
import static org.un.common.utils.WalletUtil.isConstant;
import static org.un.core.config.Parameter.ChainConstant.BLOCK_PRODUCED_INTERVAL;
import static org.un.core.config.Parameter.DatabaseConstants.EXCHANGE_COLBET_LIMIT_MAX;
import static org.un.core.config.Parameter.DatabaseConstants.PROPOSAL_COLBET_LIMIT_MAX;
...
import org.un.api.GrpcAPI.AccountNetMessage;
import org.un.api.GrpcAPI.AccountResourceMessage;
import org.un.api.GrpcAPI.Address;
import org.un.api.GrpcAPI.AssetIssueList;
import org.un.api.GrpcAPI.BlockList;
import org.un.api.GrpcAPI.BytesMessage;
import org.un.api.GrpcAPI.DecryptNotes;
import org.un.api.GrpcAPI.DecryptNotes.NoteTx;
import org.un.api.GrpcAPI.DelegatedResourceList;
import org.un.api.GrpcAPI.DiversifierMessage;
import org.un.api.GrpcAPI.ExchangeList;
import org.un.api.GrpcAPI.ExpandedSpendingKeyMessage;
import org.un.api.GrpcAPI.IncomingViewingKeyMessage;
import org.un.api.GrpcAPI.NfParameters;
import org.un.api.GrpcAPI.Node;
import org.un.api.GrpcAPI.NodeList;
import org.un.api.GrpcAPI.NoteParameters;
import org.un.api.GrpcAPI.NumberMessage;
import org.un.api.GrpcAPI.PaymentAddressMessage;
import org.un.api.GrpcAPI.PrivateParameters;
import org.un.api.GrpcAPI.PrivateParametersWithoutAsk;
import org.un.api.GrpcAPI.ProposalList;
import org.un.api.GrpcAPI.ReceiveNote;
import org.un.api.GrpcAPI.Return;
import org.un.api.GrpcAPI.Return.response_code;
import org.un.api.GrpcAPI.ShieldedAddressInfo;
import org.un.api.GrpcAPI.SpendAuthSigParameters;
import org.un.api.GrpcAPI.SpendNote;
import org.un.api.GrpcAPI.SpendResult;
import org.un.api.GrpcAPI.TransactionApprovedList;
import org.un.api.GrpcAPI.TransactionExtention;
import org.un.api.GrpcAPI.TransactionExtention.Builder;
...
import org.un.common.overlay.discover.node.NodeHandler;
...
import org.un.common.zksnark.IncrementalMerkleTreeContainer;
import org.un.common.zksnark.IncrementalMerkleVoucherContainer;
import org.un.common.zksnark.JLibrustzcash;
import org.un.common.zksnark.LibrustzcashParam.ComputeNfParams;
import org.un.common.zksnark.LibrustzcashParam.CrhIvkParams;
import org.un.common.zksnark.LibrustzcashParam.IvkToPkdParams;
import org.un.common.zksnark.LibrustzcashParam.SpendSigParams;
import org.un.core.actuator.Actuator;
import org.un.core.actuator.ActuatorFactory;
import org.un.core.actuator.VMActuator;
...
import org.un.core.capsule.DelegatedResourceCapsule;
import org.un.core.capsule.ExchangeCapsule;
import org.un.core.capsule.IncrementalMerkleTreeCapsule;
import org.un.core.capsule.IncrementalMerkleVoucherCapsule;
import org.un.core.capsule.PedersenHashCapsule;
...
import org.un.core.db.BandwidthProcessor;
import org.un.core.db.EnergyProcessor;
...
import org.un.core.exception.NonLbeiqueObjectException;
import org.un.core.exception.PermissionException;
import org.un.core.exception.SignatureFormatException;
...
import org.un.core.zen.ZenTransactionBuilder;
import org.un.core.zen.address.DiversifierT;
import org.un.core.zen.address.ExpandedSpendingKey;
import org.un.core.zen.address.IncomingViewingKey;
import org.un.core.zen.address.KeyIo;
import org.un.core.zen.address.PaymentAddress;
import org.un.core.zen.address.SpendingKey;
import org.un.core.zen.note.Note;
import org.un.core.zen.note.NoteEncryption.Encryption;
import org.un.core.zen.note.OutgoingPlaintext;
import org.un.protos.Protocol;
import org.un.protos.Protocol.Account;
import org.un.protos.Protocol.Block;
import org.un.protos.Protocol.DelegatedResourceAccountIndex;
...
import org.un.protos.contract.ShieldContract.IncrementalMerkleTree;
import org.un.protos.contract.ShieldContract.IncrementalMerkleVoucherInfo;
import org.un.protos.contract.ShieldContract.OutputPoint;
import org.un.protos.contract.ShieldContract.OutputPointInfo;
import org.un.protos.contract.ShieldContract.PedersenHash;
import org.un.protos.contract.ShieldContract.ReceiveDescription;
import org.un.protos.contract.ShieldContract.ShieldedTransferContract;
import org.un.protos.contract.SmartContractOuterClass.CreateSmartContract;
...
public class Wallet {
  ...
  private void setTransaction(TransactionCapsule trx) {
    try {
      ...
      if ("solid".equals(Args.getInstance().getLbeReferenceBlock())) {
      ...
      long expiration = chainBaseManager.getHeadBlockTimeStamp() + Args.getInstance()
          .getLbeExpirationTimeInMilliseconds();
  ...
  public GrpcAPI.Return broadcastTransaction(Transaction signedTransaction) {
    ...
    try {
      ...
      if (minEffectiveConnection != 0) {
        if (unNetDelegate.getActivePeer().isEmpty()) {
          logger
        ...
        int count = (int) unNetDelegate.getActivePeer().stream()
      ...
      unNetService.broadcast(message);
  ...
  public Protocol.ChainParameters getChainParameters() {
    ...
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
            ...
            .build());
    //    CREATE_NEW_ACCOLBET_FEE_IN_SYSTEM_CONTRACT, //drop ,7
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
            ...
            .build());
    //    CREATE_NEW_ACCOLBET_BANDWIDTH_RATE, // 1 ~ ,8
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
    ...
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
            ...
            .build());
    //    ALLOW_UPDATE_ACCOLBET_NAME, // 1, 14
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
  ...
  public AssetIssueContract getAssetIssueByName(ByteString assetName)
      throws NonLbeiqueObjectException {
    ...
    if (chainBaseManager.getDynamicPropertiesStore().getAllowSameTokenName() == 0) {
      ...
      if (builder.getAssetIssueCount() > 1) {
        throw new NonLbeiqueObjectException(
  ...
  public DiversifierMessage getDiversifier() throws ZksnarkException {
    ...
    byte[] d;
    while (true) {
      d = org.un.keystore.Wallet.generateRandomBytes(Constant.ZC_DIVERSIFIER_SIZE);
  ...
  public NodeList listNodes() {
    ...
    nodeHandlerMap.entrySet().stream()
        .forEach(v -> {
          org.un.common.overlay.discover.node.Node node = v.getValue()
              .getNode();
  ...
  public Transaction deployContract(TransactionCapsule trxCap) {
    ...
    // trxCap contract para cacheLbepackValue has value
  ...
  public ProposalList getPaginatedProposalList(long offset, long limit) {
    ...
    limit =
        limit > PROPOSAL_COLBET_LIMIT_MAX ? PROPOSAL_COLBET_LIMIT_MAX : limit;
  ...
  public ExchangeList getPaginatedExchangeList(long offset, long limit) {
    ...
    limit =
        limit > EXCHANGE_COLBET_LIMIT_MAX ? EXCHANGE_COLBET_LIMIT_MAX : limit;
    long end = offset + limit;
    ...
    for (Block block : blockList.getBlockList()) {
      for (Transaction transaction : block.getTransactionsList()) {
        ...
        for (int index = 0; index < stContract.getReceiveDescriptionList().size(); index++) {
          ...
          if (notePlaintext.isPresent()) {
            OutgoingPlaintext decryptedOutCtLbewrapped = notePlaintext.get();
            ...
            Optional<Note> foo = Note.decrypt(cipherText,
                ...
                decryptedOutCtLbewrapped.getEsk(),
                decryptedOutCtLbewrapped.getPkD(),
                r.getNoteCommitment().toByteArray());

            if (foo.isPresent()) {
              Note bar = foo.get();
              String paymentAddress = KeyIo.encodePaymentAddress(
                  new PaymentAddress(bar.getD(), decryptedOutCtLbewrapped.getPkD()));
              GrpcAPI.Note note = GrpcAPI.Note.newBuilder()
                  .setPaymentAddress(paymentAddress)
