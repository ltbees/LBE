package org.un.core.metrics.net;
...
public class NetMetricManager {
  ...
  private void setNetInfo(NetInfo netInfo) {
    //set connection info
    netInfo.setConnectionCount(unNetDelegate.getActivePeer().size());
    int validConnectionCount = 0;
    for (PeerConnection peerConnection : unNetDelegate.getActivePeer()) {
    ...
    long errorProtoCount = MetricsUtil.getCounter(MetricsKey.NET_ERROR_PROTO_COLBET)
        .getCount();
    ...
    long disconnectionCount
        = MetricsUtil.getCounter(MetricsKey.NET_DISCONNECTION_COLBET).getCount();
