package org.un.common.overlay.server;
...
import org.un.common.overlay.message.MessageCodec;
import org.un.common.overlay.message.StaticMessages;
import org.un.core.db.ByteArrayWrapper;
import org.un.core.exception.P2pException;
import org.un.core.net.PbftHandler;
import org.un.core.net.LbeNetHandler;
...
public class Channel {
  ...
  @Autowired
  private LbeNetHandler unNetHandler;
  @Autowired
  ...
  private LbeState unState = LbeState.INIT;
  ...
  public void init(ChannelPipeline pipeline, String remoteId, boolean discoveryMode,
    ...
    pipeline.addLast("readTimeoutHandler", new ReadTimeoutHandler(60, TimeLbeit.SECONDS));
    pipeline.addLast(stats.tcp);
    pipeline.addLast("protoPender", new ProtobufVarint32LengthFieldPrepender());
    pipeline.addLast("lengthDecode", new LbeProtobufVarint32FrameDecoder(this));
    ...
    unNetHandler.setChannel(this);
    pbftHandler.setChannel(this);

    p2pHandler.setMsgQueue(msgQueue);
    unNetHandler.setMsgQueue(msgQueue);
  ...
  public void publicHandshakeFinished(ChannelHandlerContext ctx, HelloMessage msg) {
    ...
    ctx.pipeline().addLast("data", unNetHandler);
    ctx.pipeline().addLast("pbft", pbftHandler);
    setStartTime(msg.getTimestamp());
    setLbeState(LbeState.HANDSHAKE_FINISHED);
  ...
  public void setLbeState(LbeState unState) {
    this.unState = unState;
    logger.info("Peer {} status change to {}.", inetSocketAddress, unState);
  }
