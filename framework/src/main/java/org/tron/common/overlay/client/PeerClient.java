...
import org.un.common.overlay.server.LbeChannelInitializer;
...
public class PeerClient {
  ...
  public PeerClient() {
    workerGroup = new NioEventLoopGroup(0, new ThreadFactory() {
      ...
      public Thread newThread(Runnable r) {
        return new Thread(r, "LbeJClientWorker-" + cnt.getAndIncrement());
      }
    });
  }
  ...
  private ChannelFuture connectAsync(String host, int port, String remoteId,
    ...
    LbeChannelInitializer unChannelInitializer = ctx
        .getBean(LbeChannelInitializer.class, remoteId);
    unChannelInitializer.setPeerDiscoveryMode(discoveryMode);
    ...
    b.handler(unChannelInitializer);
  ...
  public void close() {
    ...
    workerGroup.terminationFuture().syncLbeinterruptibly();
  }
}
